...
4. Narrative instruction: two explicit sections — `## Narrative Summary` (5–8 paragraphs) and `## Key Takeaways` (3–5 actionable items). The model is told not to fold an actionable paragraph into the Narrative Summary; it belongs only in Key Takeaways. The PDF renderer's `_draw_narrative_paginated` already detects `#`-prefixed lines and renders them as styled section headings, so no PDF-side changes are needed when the LLM emits these headers.
