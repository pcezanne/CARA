...
**Show Tags dialog** (`app/views/dialogs/show_tags_dialog.py`, class `ShowTagsDialog`): each row represents one (move, preset) pair. Three columns: board miniature (played move arrow only — best-move rendering deferred, no existing two-arrow widget), category checkboxes (blank for 3x3), and why-note text. All fields are editable on open; **OK** persists changes to the in-memory cache only (same two-step save pattern as Tag This Moment — user must still Ctrl+Alt+L to write to PGN). **Cancel** discards edits. Zero-category saves (`cat=""`) round-trip correctly through OK — unchecking all boxes but keeping why text preserves the moment as a `cat=""` entry, matching `MomentDialog.get_entries()` semantics. Best-move arrow deferred — `MiniChessBoardWidget` only supports one arrow; adding a two-arrow variant is a future design pass.
...
1. `## Glossary — <preset>` block for each preset present in the data (verbatim definitions only — never paraphrase or invent). CLAMP has full text (C=Checks, L=Loose Pieces and Squares, A=Alignments, M=Mobility Restrictions, P=Passed Pawns). CCT/3x3/Custom are TODO placeholders in `_PRESET_GLOSSARIES` — add verbatim text when Paul supplies it; leave as `""` until then.
...
4. Narrative instruction: 3–5 paragraphs (no step 2 / no "Also flagged").

**Narrative panel controls** (visible when AI is configured): "Also flag shallow notes" checkbox (default checked) placed above the Generate button — **currently inert** (the checkbox state is plumbed through but `include_also_flagged` is ignored by `build_prompt`; Paul plans a separate "Log Tags" replacement for this feature). Model combo (ephemeral, lists models for the active provider from `ai_models.{provider}.models`), Timeout spinner (10–600s, shared with AI Summary via `ai_summary.request_timeout_seconds` — changing it here also changes it in the AI Summary tab), Tokens spinner (256–16000, ephemeral). The controller's `get_available_models()` / `get_default_narrative_model()` / `get_narrative_timeout_seconds()` drive the view; `set_narrative_*()` setters update the in-memory state (timeout also persists).
...
**Tags Report panel** (always visible, between the charts container and the Narrative Summary panel): shows every tagged (move, preset) pair across all games returned by the current Data Source, grouped by database (header only when >1 database is in scope) then by game. **Player selection is required** — the report shows "Select a player to view tagged moments." until the user makes an explicit choice in the Player dropdown (same gate as the charts above). Live-editable via `_TagRowWidget.edited` signal — edits land immediately in `ChessLogController._multi_cache` without an OK button. When there are more than 9 rows the panel wraps them in an inner `QScrollArea` at a fixed height of 500px so the outer scroll area is not overwhelmed; at or below 9 rows the inner widget expands naturally. When the source is "None", no player is selected, or no games have tags, an empty-state label is shown instead. Rebuild is triggered on Data Source change (`_on_source_changed`) and player change (`_on_player_changed`) and on `set_controller()`. Game headers show `"{white} - {black} {result} ({date} - {moves} moves)"` (no "game Nr." prefix — that belongs to bulk analysis dialogs). Key shared helpers: `app/views/dialogs/_tag_row_helpers.py` (`ply_for_path`, `node_info`), `DatabaseModel.display_name` for the database-header name.
...
**Key files**: `app/services/chess_log_stats_service.py`, `app/services/chess_log_narrative_service.py`, `app/controllers/chess_log_charts_controller.py`, `app/controllers/chess_log_controller.py`, `app/views/detail_chess_log_charts_view.py`, `app/views/widgets/chess_log_category_chart_widget.py`, `app/views/dialogs/_tag_row_helpers.py`, `app/utils/ai_provider_config.py`.
