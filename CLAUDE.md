...
**Widget rendering**: `ChessLogCategoryChartWidget` supports three x-axis layouts via `_bin_x()`. `uniform_bins` and `gap_compressed` both currently use equal pixel spacing by bin index (`gap_compressed` falls back to `uniform_bins` — full GapCompressedTimeLayout port from `detail_player_stats_view.py` is a TODO). `calendar_linear` positions bins at their calendar-median `time_pct`. Smooth Catmull-Rom lines via `_smooth_polyline_path()` (ported from Player Stats) are used when `series.line_style == "smooth"`.

**DRY debt (future PR)**: `_smooth_polyline_path` and `GapCompressedTimeLayout` exist in both `detail_player_stats_view.py` and `chess_log_category_chart_widget.py`. These should be extracted to `app/views/widgets/_chart_layout_helpers.py` so a third chart tab doesn't produce a third copy. Similarly, `ChessLogChartsMenuController` and `PlayerStatsTimeSeriesMenuController` share ~80% structure — a `RadioMenuController` base class would remove ~150 lines of duplication. Both are deliberate additive ports; refactoring is a separate PR.
