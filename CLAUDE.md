...
**Tags Report panel** (always visible, between the charts container and the Narrative Summary panel): shows every tagged (move, preset) pair across all games returned by the current Data Source, grouped by database (header only when >1 database is in scope) then by game. Live-editable via `_TagRowWidget.edited` signal — edits land immediately in `ChessLogController._multi_cache` without an OK button. The scroll area wraps the rows only when there are more than 9 rows. When the source is "None" or no games have tags, an empty-state label is shown instead. Rebuild is triggered on Data Source change (`_on_source_changed`) and on `set_controller()`. Key shared helpers: `app/views/dialogs/_tag_row_helpers.py` (`ply_for_path`, `node_info`), `BulkAnalysisService.format_game_label` for the game-header label, `DatabaseModel.display_name` for the database-header name.
