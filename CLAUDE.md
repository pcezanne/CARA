...
4. Narrative instruction: two explicit sections — `## Narrative Summary` (5–8 paragraphs) and `## Key Takeaways` (1–3 short prose paragraphs, not a list). Key Takeaways paragraphs must be anchored to a specific verbatim quote from the player's own why-notes or whole-game notes; no generic coaching advice. The model is told not to fold an actionable paragraph into the Narrative Summary. Both surfaces render `**bold**` markdown: the PDF via `_parse_bold_spans` (never passes literal `**` to `drawText`; bold runs use `_font_body_bold`) and the on-screen panel via `QTextEdit.setMarkdown`. `##` headings render as styled section headings in both.
...
**Narrative Summary pagination**: `ChessLogPDFService._draw_narrative_paginated` word-wraps the narrative line-by-line and calls `_ensure_space` before each line, so long narratives page-break cleanly at the footer boundary rather than overflowing it. The method supports `**bold**` markdown via `_parse_bold_spans` — literal `**` markers are consumed and never reach `drawText`; bold word runs are rendered with `_font_body_bold` using `drawText(QPointF, word)` with a running x-cursor per line. The on-screen narrative panel uses `QTextEdit.setMarkdown` on success so headings and bold render natively; error and placeholder paths use `setPlainText`.
