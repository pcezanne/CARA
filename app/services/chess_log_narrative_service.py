...
_SYSTEM_PROMPT = (
    ...
    "## Tactical Breakdown\n"
    "## Key Takeaways\n\n"
    "The Patterns & Recurrent Themes section identifies 2 to 4 cross-cutting behavioral themes you see across the player's tagged moments — the underlying tendencies (rushing calculation, drifting attention in slow positions, missing prophylaxis, etc.) that show up regardless of category. Keep the tone player-focused. Do NOT frame this section around CLAMP letters or percentages; category-level detail belongs in the Tactical Breakdown section that follows. "
    "The Tactical Breakdown section MUST be a markdown pipe table with exactly three columns and a header row, in this exact format:\n\n"
    "| Area | Observed Issue | Strategic Impact |\n"
    "| --- | --- | --- |\n"
    "| ... | ... | ... |\n\n"
    "One row per CLAMP category (or CCT category / 3x3 Why-question) that the data supports. This is where CLAMP letter framing, category names, and per-category percentages belong — not in Patterns & Recurrent Themes. Do not repeat the same content across multiple rows. Do not add extra columns. Do not emit any prose outside the table in this section. "
...
_NARRATIVE_STEP = (
    "Please write:\n\n"
    "1. **Patterns & Recurrent Themes** (2 to 4 paragraphs, each covering one "
    "cross-cutting behavioral theme): synthesize what recurring tendencies you see "
    "across my tagged moments. Focus on the player-level behavior (how I seem to "
    "think, when I rush, what I overlook), not on CLAMP letters or category "
    "percentages — that granular data goes in the Tactical Breakdown table below. "
    "Reference at most 1 to 2 verbatim quotes from my own why-notes per theme.\n\n"
    "2. **Tactical Breakdown** (a markdown pipe table, not prose): exactly the header "
    "row `| Area | Observed Issue | Strategic Impact |` followed by an alignment "
    ...
    "to name a concrete, actionable next step. Do not restate the earlier sections "
    "in miniature. At most 1 to 2 verbatim quotes per paragraph.\n\n"
